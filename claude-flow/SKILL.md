...
**Optional — Workflow-tool-backed loop:** the goal-mode iterate-until-verified loop can be run by the Claude Code **Workflow tool**, which holds the iteration counter and a deterministic verification agent that *runs the success command and parses its exit code* — making the loop condition machine-checkable in script, not a small-model judgement, and turning a gamed completion into a failed code condition. Scope it to one goal span and hand back to the conversation between goals (the workflow cannot take mid-run human input). Opt-in, interactive sessions only; keep `run_manifest.py` as the durable cross-session record. See `references/workflow-tool-orchestration.md` (Template B).
...
- **Prior-art cross-reference — GitHub Spec Kit (spec-driven development):** [github/spec-kit](https://github.com/github/spec-kit) runs a parallel pipeline — `constitution → specify → plan → tasks → implement` plus optional `clarify` / `analyze` / `checklist`. The mapping to claude-flow is near-total (specify↔Phase 3, plan↔Phase 4, tasks↔Phase 4b, implement↔Phase 5, clarify↔Phase 3 ambiguities, analyze↔Phase 4c, checklist↔Phase 6), and claude-flow goes deeper (TDD, multi-model advisor, risk screen, 8 paths). The one idea claude-flow lacks a first-class equivalent for is the **constitution**: a persistent, workflow-authored project-principles artifact every later step must honor — claude-flow currently relies on per-project CLAUDE.md for this. Not actionable today; if a recurring need surfaces for workflow-enforced principles (e.g. an injected "constitution check" in Phase 4c or Phase 6), this is the gap to fill.
