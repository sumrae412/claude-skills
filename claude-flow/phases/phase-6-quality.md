...
2. **Run the selector script** — do not partition the registry by hand. Glob and content-pattern matching are mechanical:
   ```bash
   git diff --name-only HEAD~1 | python3 scripts/select_reviewers.py --workflow-path <path>
   ```
   The script loads the bundled `reviewer-registry.json`, merges optional user-global and project-local overrides, infers a `review_budget`, and returns `always`, `conditional_matched`, `conditional_skipped`, `budget_skipped`, `by_tier`, `review_budget`, `budget_reasons`, and `registry_sources`. Use `by_tier` to drive dispatch.
...
**Why a script for selection:** glob matching, content-pattern regex with thresholds, budget filtering, and tier grouping are pure mechanical work. The script eliminates that whole class of errors and frees the LLM to focus on prompt assembly, dispatch, and result triage.

Load `references/review-budgets.md` only when you need the full rationale for
low/medium/high budget selection.

**Registry format** (see `../reviewer-registry.json`):
...
**Adding project-specific reviewers:** Drop a `reviewer-registry.json` in the target project's `.claude/` directory. Project reviewers are merged with and override bundled defaults by `id`.
...
- **Budget floor matters.** Reviewers whose `min_budget` exceeds the selected `review_budget` are skipped deterministically and listed under `budget_skipped`.
...
| 2 | `safety-reviewer` | `pr-review-toolkit:silent-failure-hunter` + `security-reviewer` | sonnet | `medium` | Always — silent failures + security (combined) |
| 2 | `test-coverage-analyzer` | `pr-review-toolkit:pr-test-analyzer` | sonnet | `medium` | Always — test gaps |
| 2 | `curmudgeon-review` | `scripts/curmudgeon_review.sh` (Codex CLI) | gpt-5-codex | `high` | Always — non-Anthropic second opinion |
| 2 | `adversarial-breaker` | `general-purpose` (persona file) | sonnet | `high` | Always — scored reviewer; sub-7/10 scores become blocking findings |
...
| 3-4 | `lightweight-reviewer` | `general-purpose` (haiku) | haiku | `low` | **Batched** — single dispatch with combined checklist (see below) |
...
**CLI-backed reviewers (`runner_script`):** Entries with a `runner` field shell out to a local script instead of dispatching an agent. Use the selector's `resolved_runner_script`, not the raw relative path. Full schema in `references/reviewer-registry-schema.md`.
...
if jq -e '.runner == "codex-cli"' <<<"$reviewer" >/dev/null; then
    bash "$(jq -r .resolved_runner_script <<<"$reviewer")" "$DIFF_FILE"
fi
```
...
After each reviewer completes, if its registry entry has `score_threshold`, run:
    ...
    python3 scripts/aggregate_reviewer_findings.py --reviewer <output.json> --registry reviewer-registry.json
