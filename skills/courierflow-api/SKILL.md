...
**Never use `BaseHTTPMiddleware`** — its `call_next()` wraps response bodies in a task group that breaks `AsyncSession` lifecycle. All middleware must use the pure ASGI pattern:
...
class MyMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        if scope["type"] != "http":
            ...
            return
        # To inject response headers, wrap send:
...
**Invariant:** `grep -r "BaseHTTPMiddleware" app/` must return zero hits.
