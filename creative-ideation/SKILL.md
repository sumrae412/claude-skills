...
description: Use when the user needs open-ended idea generation or selection help — "I want to make/build/write/start something", "I'm stuck", "inspire me", "make this weirder", "help me pick between these", "I need to invent X", naming a product/campaign, or unsticking a stale creative approach.
user-invocable: true
---
...
This skill is a **method library for divergent idea generation** — naming, campaigns, product ideas, unsticking a stale approach, picking between options. It assumes the creative *target* is already roughly known and the gap is idea quality/quantity/angle.

It is NOT for requirements discovery on a feature build — exploring what a user actually needs, scoping a build, or clarifying intent before implementation. That belongs to `brainstorming` (or `claude-flow` for full builds). If the request is "help me figure out what to build" in a software-feature sense rather than "help me generate ideas," route there instead.
...
- `references/methods/` — 20 named methods, one file each, load only the one you're using

---

## Attribution

Ported from [github.com/NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent), path `optional-skills/creative/creative-ideation/`. Original author SHL0MS, MIT license, version 2.1.0. Ported into this repo 2026-07-04.
