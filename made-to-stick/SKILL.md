---
name: made-to-stick
description: "Craft messages that are understood, remembered, and drive action using the SUCCESs framework (Simple, Unexpected, Concrete, Credible, Emotional, Stories). Use when the user mentions 'make it memorable', 'sticky messaging', 'tagline', 'value proposition', 'why the message isn't landing', 'curse of knowledge', 'concrete language', 'people forget our message', 'make this stick', or 'no one remembers our pitch'. Also trigger when writing a pitch deck, simplifying a complex product explanation, or making a presentation more compelling and memorable."
user-invocable: true
---
...
> **Communication principles:** Load `shared/communication-principles.md` before drafting. Run the §9 sameness-detector pass before delivering any final copy.

**Output workspace:** Write all output files to `~/Documents/<project>-marketing/` - never inside a tracked git repo. Example: `~/Documents/acme-marketing/sticky-messaging.md`. When reading context from product-marketing, check `~/Documents/<project>-marketing/.agents/product-marketing.md` if it exists.
...
**Core concept:** Find the core of the idea and share it compactly. Simple does not mean dumbed down - it means ruthless prioritization: "if you say three things, you say nothing."
...
**Application:** features to outcomes; percentages to real numbers ("saves 40%" to "saves 16 hours/month"); categories to specific examples ("restaurants" to "pizza shops in Brooklyn"); demos over feature lists.
...
| **Self-interest** | "What's in it for me?" | WIIFM: features to personal benefits |
...
- `storybrand-messaging` - narrative brand framework for customer-as-hero messaging
- `copywriting` - homepage and landing page copy application
