...
**Communication Principles:** Before drafting any positioning copy, load `/Users/summerrae/claude_code/claude-skills/shared/communication-principles.md`. Run the §9 sameness-detector pass on all tagline candidates and one-liner variants before presenting them.

## Output Workspace

**Write all output files to `~/Documents/<project-name>-positioning/`** (e.g., `~/Documents/charley-bot-positioning/`). Never write inside a git repo, the current working directory of a tracked project, or any path that could dirty a `git status`. Create the workspace directory if it does not exist.
...
Save to `<workspace>/intake.md` — a brief summary of the product, problem, alternatives, and customers. If built on prior session data, note the source files used. Project name: kebab-case (e.g., `charley-bot`).

Create `<workspace>/PROGRESS.md` with: project name, skill name (`startup-positioning`), start date, language, research mode (Live / Knowledge-Based), and a phase checklist. Update it after each phase completes.
...
All files write to `<workspace>/` and `<workspace>/raw/` — never to a git-tracked project directory.

**`<workspace>/positioning-doc.md`** — The main deliverable:
...
**`<workspace>/positioning-statement.md`** — Statements and messaging:
...
**`<workspace>/competitive-alternatives.md`** — Complete alternatives map:
...
**`<workspace>/market-category-analysis.md`** — Category strategy:
...
**`<workspace>/messaging-implications.md`** — Bridge from positioning to copy:
...
Each agent saves its raw output to `<workspace>/raw/`. The synthesis phase reads these raw files and produces the polished deliverables above. Agents must NOT write directly to deliverable paths — raw and synthesized output are separate.
...
After all positioning deliverables are written, run a two-pass verification.

> **Reference:** Read `references/verification-agent.md` for the full verification protocol, universal checks, skill-specific checks, and the V2 adversarial pass.

### Process

1. Spawn agent **V1: Verification** — it reads all deliverable files and checks for: unlabeled claims, internal contradictions, confidence rating consistency, missing data gaps, missing flags, stale data, and duplicate-source false corroboration. Session provenance counts as duplication: a survey generated by a session primed with the study's own corpus is NOT independent corroboration — confirm fresh-session provenance before any "two independent evidence bases" language.
...
3. Spawn agent **V2: Adversarial Fact-Check** — V1 reads deliverables only and is insufficient for load-bearing external claims. For each claim the Onliness/differentiator statement depends on (competitor-capability counter-examples, statute dates, case-law framings, pricing anchors), V2 runs a refutation-oriented check against primary sources (statute text, help docs, pricing pages, the actual case). Validated 2026-07-11 on the charley.bot positioning study: V2 refuted three claims V1 had certified clean.
4. V1 + V2 findings go into `<workspace>/verification-report.md`
5. **Propagation:** when verification corrects a deliverable, correct or banner the matching `raw/` evidence files in the same pass — downstream authors write from `raw/` and re-import refuted claims otherwise (see the Propagation Rule in `references/verification-agent.md`)
6. **If Critical issues found (any V2 refutation is Critical):** Pause and present issues to the user. Ask: fix first, or proceed as-is?
7. **If only Warnings/Info:** Show one-line summary

In Claude.ai or when Agent tool is unavailable, run both passes yourself in the main conversation following the same protocol.
...
| `verification-agent.md` | After synthesis | ~172 | Verification protocol: V1 universal + skill-specific checks, V2 adversarial fact-check, raw/ propagation rule |
