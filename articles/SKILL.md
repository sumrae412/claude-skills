---
name: articles
description: Use when the user says "/articles", "check my articles", "process Claude articles", "triage my reading queue", or asks whether anything in their Mem reading queue is worth pulling into their projects or skills. Pulls unread notes from Mem's "Claude articles" collection ONLY, runs each through `useful-for` against the user's claude-skills repo and active projects (CourierFlow, BetterBurgh, DeepLearning.ai work), and moves processed notes to "Claude articles — reviewed". Video captures (any 🎥 Mem clip, plus YouTube, Vimeo, Loom, TikTok, X links) get automatic transcript analysis before the verdict, and captures whose payload is a screenshot or PDF get an attachment-read. Triggers on any phrasing about checking, processing, triaging, or reviewing the reading inbox in Mem.
user-invocable: true
---
...
2. **One agent per group, all dispatched in ONE parallel block.** Use a `general-purpose` executor with an explicit down-tier model override (sonnet) — never let a swarm agent inherit the orchestrator's model. Each brief is self-contained and MUST include:
   ...
   - The triage-mode rules that bite downstream, restated: body-less captures = LOW-confidence title-level verdicts; known-failure tracking links (Half Baked, Beehiiv) = never fetch; attachment-carrying captures = the agent reads the attachment itself via `answer_question_about_attachment` before its verdict (screenshots on technical X posts are the payload, not decoration — the note body only describes them); code-repo links = the agent does the source-read itself, inline in its own foreground; video links = UNCONDITIONAL transcript pull by the agent itself (detect shape-first: a `🎥` clip fingerprint in the note body counts as a video capture even when the host isn't a known video domain; `watch` skill, `--detail transcript`, canonical URL resolved by title-search when the capture has only a redirect), inline in its own foreground — and a caption-less video gets a title-level LOW-confidence verdict, never a paid Whisper call (that offer belongs to the orchestrator).
...
2. Run the `useful-for` mental model (see `useful-for/SKILL.md`) against the priority target list above. Six modes, in precedence order — **a note that matches more than one takes the LOWEST bullet it matches**, because the source-reading modes exist precisely to rescue captures the shallower modes would mis-rate. A body-less capture linking a YouTube video is a video transcript-read, not a body-less title guess; an X post whose payload is a screenshot is an attachment-read, not an inline-summary triage:
   ...
   - **Attachment-read** (the capture's real payload is a screenshot, PDF, or other attachment — common on technical X/Twitter posts, 7 of 24 X captures in the 2026-07-25 inbox): Mem's note body *describes* an attachment ("a screenshot of a technical document providing a ranking table…") instead of transcribing it, so the adoptable detail is invisible to inline triage. When a note carries an attachment AND its summary implies textual or structured content — code, config, a table, a checklist, a rubric, slides, a document — read the attachment before the verdict. Mem's own tools do this locally, free, with no external fetch and no auth:
     - `answer_question_about_attachment` with `attachment_kind` + `attachment_id` — one focused pull ("transcribe the text verbatim, including table rows and headers"). Preferred: cheapest, returns exactly what's needed.
     - `read_attachment` for structured/paginated extraction (multi-page PDFs, long OCR).
     - `attachment_id` and `attachment_kind` come from `get_note`'s attachment metadata; Mem also embeds them in the image URL fragment (`…#attachment_id=<uuid>&attachment_summary=…`), so a body-only capture still yields the id.

     Skip the read when the attachment summary implies a purely decorative image (a banner, a logo, a headshot, a stock photo, an infographic already fully described). Validated 2026-07-25: an X capture of a model-routing post summarized only as "a ranking table for choosing AI models based on cost, intelligence, and taste" yielded, on attachment-read, the complete 4-model scoring table plus nine application rules — directly adoptable against `model-router` and none of it visible in the Mem summary.
   - **Body-less capture** (bare title + source + tracking-redirect URL, no Mem TL;DR/Key Points — was the dominant shape through mid-2026, now the minority: 6 of 73 notes on 2026-07-25, since Mem's clip rules began emitting structured summaries): there is nothing to fetch (redirects are dead — see Rules) and nothing to summarize. Triage from the title at LOW confidence and route to the verify-before-author rule below for any item you'd otherwise call "high-value." If several notes in a run are body-less, say so once at the top of the run ("most captures are title-only — verdicts are title-level guesses; I'll verify any high-value pulls against canonical sources before acting") so the user reads the rollup with correct confidence.
   - **Video-link transcript-read** — UNCONDITIONAL for any video capture. Detect by capture SHAPE first, domain second — video hosts are open-ended, so a domain allowlist alone under-catches. A note is a video capture when ANY of these fire, checked in order:
     1. **Shape (primary):** the note body carries Mem's video-clip fingerprint — a `🎥` in the header/title line. (A `Video Duration:` stat line is corroborating but appears on only some clips — never require it.)
     2. **Domain (secondary):** the URL (or resolved canonical URL) matches `youtube.com`, `youtu.be`, `vimeo.com`, `loom.com`, `tiktok.com`, or an `x.com`/`twitter.com` video post. This catches notes that *link* a video without being Mem video-clip captures. The list is known-incomplete — e.g. `watch.getcontrast.io` hosted a real 38-min talk that only the 🎥 shape caught (validated 2026-07-24 across a 73-note inbox sweep: 5/5 video clips carried 🎥; the allowlist alone missed 1 of 5).
     3. **Marker (tertiary):** a `Type: video` body marker (Mem clip rules, 2026-07-19). Zero notes carried it in the 2026-07-24 sweep — treat it as a bonus signal, never the load-bearing one.

     Do NOT gate on whether the title "plausibly maps to a target": a Mem video capture holds title + description only, the content lives in the audio, and captions are a free fetch — so pull the transcript FIRST, then run the useful-for verdict against the transcript. Use the `watch` skill ([claude-video plugin](https://github.com/bradautomates/claude-video)) in transcript-only mode:
...
- **A Mem summary of a code repo is necessary triage but not sufficient — code-repo / installable-skill links get a source-read before a final verdict, because summaries systematically under-surface adoptable detail.** The inline and body-less modes are correct for articles, tweets, newsletters, and blog posts; they are wrong for repos. Validated 2026-06-26: summary-level triage rated four code repos "some-value/already-covered"; source-reads found a bug-fix repro-loop gate (now [claude-skills #176](https://github.com/sumrae412/claude-skills/pull/176)), a new `codebase-design` skill ([#177](https://github.com/sumrae412/claude-skills/pull/177)), a review-pr two-axis structural split ([#178](https://github.com/sumrae412/claude-skills/pull/178)), and design-polish specs ([#179](https://github.com/sumrae412/claude-skills/pull/179)) — none visible in the summaries. Do NOT source-read news articles, tweets, or blog posts **as prose** — their Mem summary is the right tool for the words, and dead redirect URLs cannot be fetched (the known-failure-link rule still governs those). Two exceptions override that, and both are decided by capture shape, not by source type:

  1. **Video captures** get a transcript-read (step 2's video-link mode), because a Mem capture of a video holds the title/description only — the content is in the audio, and captions are a free fetch.
  2. **Attachment-carrying captures** get an attachment-read (step 2's attachment-read mode). This is what most often rescues X/Twitter posts: the rule above bars source-reading the *tweet text*, but says nothing about the screenshot the tweet exists to share — and on technical X posts the screenshot IS the payload. A tweet whose Mem summary describes an image containing code, a table, a config, or a rubric is an attachment-read, full stop.

  **X-specific routes**, when the tweet text itself needs resolving (a thread, a quoted post, a truncated body) and the Mem body is not enough: the syndication endpoint `https://cdn.syndication.twimg.com/tweet-result?id=<ID>&token=x` returns clean JSON with no auth — verified working 2026-07-25. It needs a REAL tweet id parsed from the capture's `x.com/<user>/status/<ID>` URL; a wrong or invented id silently returns an HTML shell rather than an error, so never conclude "the endpoint is broken" without re-testing on an id you actually read from the note. Long-form **X Articles** are the one case no unauthenticated route reaches — those need the user's logged-in Chrome (`claude-in-chrome` `navigate` + `get_page_text`). Full route map: `~/claude_code/agent-vault/agent/mcp-tools.md`.
