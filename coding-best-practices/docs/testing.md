...
- **`vi.spyOn<T, M>` chokes on `Console` + method-name string.** `vi.spyOn<Console, "warn">` produces `Type '"warn"' does not satisfy the constraint '"Console"'` — the `M extends Methods<Required<T>>` constraint resolves oddly when M is the method-name string against the global `Console` type. Escape hatch: import `MockInstance` from vitest and annotate the holding variable directly:
