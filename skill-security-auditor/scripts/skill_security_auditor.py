...
def clone_repo(url: str, skill_name: Optional[str] = None, cleanup: bool = False):
    ...
    tmp_dir = tempfile.mkdtemp(prefix="skill-audit-", dir=str(system_tmp))
    if not Path(tmp_dir).resolve().is_relative_to(system_tmp):
        print(f"Refusing unsafe tmp dir outside {system_tmp}: {tmp_dir}", file=sys.stderr)
        sys.exit(1)
    try:
        subprocess.run(
        ...
        if Path(tmp_dir).resolve().is_relative_to(system_tmp):
...
def main():
    ...
    try:
        ...
        if cleanup_dir:
            system_tmp = Path(tempfile.gettempdir()).resolve()
            if Path(cleanup_dir).resolve().is_relative_to(system_tmp):
                shutil.rmtree(cleanup_dir, ignore_errors=True)
            else:
                print(f"Skipping cleanup of {cleanup_dir} (outside {system_tmp})", file=sys.stderr)
